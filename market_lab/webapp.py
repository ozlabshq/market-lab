...
def build_tsmom_snapshot() -> dict:
    from .broker import load_portfolio
    from .signals import generate_tsmom_signal
    if not TSMOM_STATE.exists():
        return {"active": False, "message": "tsmom tracking not started yet"}
    ...
    pos_qty = pos.quantity if pos else 0
    weight = (pos_qty * price) / equity if equity > 0 else 0.0
    sig = generate_tsmom_signal("SPY", bars) if bars else None
    fills = 0
...
class MarketLabDashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/api/vt-trend-snapshot":
            ...
            return
        if parsed.path == "/api/tsmom-snapshot":
            _json_response(self, build_tsmom_snapshot())
            return
