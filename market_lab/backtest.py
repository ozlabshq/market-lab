...
def _stats(symbol: str, strategy: str, bars: list[Bar], equity_curve: list[float], trades: int, initial_cash: float, benchmark_start_index: int) -> BacktestResult:
    final_equity = equity_curve[-1] if equity_curve else initial_cash
    if initial_cash <= 0:
        total_return = 0.0
    else:
        total_return = final_equity / initial_cash - 1
    benchmark_base = bars[benchmark_start_index].close if bars and benchmark_start_index < len(bars) and bars[benchmark_start_index].close > 0 else bars[0].close if bars else 1.0
...
def run_signal_backtest(symbol: str, bars: list[Bar], signal_func: SignalFunction, min_history: int = 120, initial_cash: float = 10_000.0, execution: ExecutionModel | None = None) -> BacktestResult:
    """Generic long-only signal backtest.

    Decision uses bars through close of day t. Fill occurs at next bar open t+1.
    This is intentionally simple and auditable; it is not a live-trading simulator.
    """
    ...
    for i in range(min_history, len(bars) - 1):
        if pending:
            ...
            if pending == "BUY" and qty == 0 and cash > execution.commission_per_trade:
                ...
                trades += 1
            pending = None
        equity_curve.append(cash + qty * bars[i].close)
    ...
    return _stats(symbol, strategy_name, bars, equity_curve, trades, initial_cash, min_history)


def moving_average_cross_backtest(symbol: str, bars: list[Bar], fast: int = 20, slow: int = 50, initial_cash: float = 10_000.0, fee_bps: float = 5.0) -> BacktestResult:
    if len(bars) < slow + 2:
    ...
    final_equity = cash + qty * bars[-1].close
    equity_curve.append(final_equity)
    return _stats(symbol, "ma_cross", bars, equity_curve, trades, initial_cash, slow)
