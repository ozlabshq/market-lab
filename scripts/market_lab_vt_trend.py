...
def _candidate_from_vt_trend_signal(
...
def _load_or_init_portfolio(path: Path) -> Portfolio:
    ...
    portfolio = Portfolio(cash=VT_TREND_STARTING_CASH, positions={})
...
def _tracking_start_date(portfolio: Portfolio, ledger_path: Path) -> date | None:
    ...
    # For simplicity, we store start_date as a sidecar or compute from ledger
...
def _render_vt_trend_report(
    ...
    drawdown = evidence.get("drawdown", 0.0)
    dd_level = evidence.get("drawdown_level", 0)
    trend_up = evidence.get("trend_up", False)
    reentry_ok = evidence.get("reentry_ok", False)
    ...
    lines = [
        f"# vt_trend Independent Mock Tracking — {now}",
        "",
        ...
        f"| Drawdown from 90d peak | {drawdown*100:.1f}% |",
        f"| Drawdown level | {dd_level} |",
        f"| Trend regime | {'up' if trend_up else 'break'} |",
        f"| Re-entry allowed | {'yes' if reentry_ok else 'no'} |",
    ...
    lines += [
        ...
        "- Separate vt_trend ledger/state (isolated from main portfolio)",
        "- No ensemble wiring",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run vt_trend independent mock tracking")
    ...
    if args.require_live_data and _source_is_synthetic(source):
        print(f"Refusing vt_trend tracking because {symbol} data is synthetic ({source})", file=sys.stderr)
        return 1

    price = bars[-1].close
    signal_date = bars[-1].date.isoformat()

    portfolio = _load_or_init_portfolio(VT_TREND_STATE)
    sig = generate_vt_trend_signal(symbol, bars)

    # Queue candidate if weight differs
    candidate = _candidate_from_vt_trend_signal(sig, signal_date, portfolio, price)
    if candidate:
        existing = load_order_candidates(path=VT_TREND_CANDIDATES)
        existing.append(candidate)
        save_order_candidates(_dedupe_candidates(existing), path=VT_TREND_CANDIDATES)

    # Execute due candidates
    decisions, remaining = _execute_due_candidates(
        bars, {symbol: price}, VT_TREND_STATE, VT_TREND_LEDGER,
    ...
    start_date = _tracking_start_date(portfolio, VT_TREND_LEDGER)
    ...
    report_path = VT_TREND_REPORT_DIR / f"vt_trend-{stamp}.md"
